...
class GeminiService {
  ...
  Future<PlanningResult> plan({
    ...
    if (text == null || text is! String) {
      throw Exception('Gemini no devolvió texto válido. Respuesta: ${response.body}');
    }
  ...
  "tasks": [
    {
      ...
      "startAt": "2026-06-17T12:00:00-06:00",
      "endAt": "2026-06-17T13:00:00-06:00",
      "durationMinutes": 60,
      "priority": "medium",
      "status": "pending",
      "isFlexible": true,
...
- Las fechas deben ir en ISO 8601 completo, por ejemplo: 2026-06-17T14:00:00-06:00.
...
- Si el usuario dice "hoy", usa la fecha de currentDateTime.
- Si el usuario dice "mañana", usa el día siguiente a currentDateTime.
  ...
  void _validatePlanningJson(Map<String, dynamic> json) {
    ...
    for (final item in json['tasks'] as List) {
      ...
      if (item['startAt'] != null) {
        DateTime.parse(item['startAt'] as String);
      }

      if (item['endAt'] != null) {
        DateTime.parse(item['endAt'] as String);
      }

      if (item['deadline'] != null) {
        DateTime.parse(item['deadline'] as String);
      }
    }
  }
